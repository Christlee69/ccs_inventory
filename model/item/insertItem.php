<?php
    ...
    // $codeContents = 'itemDetailsItemName';
    ...
    // $urlRelativeFilePath = $tempDir.$fileName.$fileName;
    ...
    // echo '<img src="'.$qrcode.'" />';
		...
		$insertItemSql = 'INSERT INTO item(itemNumber, itemName, itemlocation, stock, unitPrice, category, description, itemdate, suggestions) VALUES(:itemNumber, :itemName, :itemlocation, :stock, :unitPrice, :category, :description, :itemdate, :suggestions)';
		$insertItemStatement = $conn->prepare($insertItemSql);
		$insertItemStatement->execute(['itemNumber' => $itemNumber, 'itemName' => $itemName, 'itemlocation' => $itemlocation, 'stock' => $quantity, 'unitPrice' => $unitPrice, 'category' => $category, 'description' => $description, 'itemdate' => $itemdate, 'suggestions' => $suggestions]);
